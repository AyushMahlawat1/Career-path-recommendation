...
import { calculateCourseScores } from '../services/scoring.service';
import { generateNextQuestion, extractTraits, generateAnalysisReportAndRoadmap } from '../services/ai.service';
...
export async function postAnswerAndGetNext(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const nextAIQuestion = await generateNextQuestion(chatHistory);
...
export async function completeAssessment(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const extracted = await prisma.$transaction(async (tx) => {
      const traitResult = await extractTraits(chatHistory);
    ...
    const courseRankings = await calculateCourseScores(assessmentId);
    ...
    const aiAnalysis = await generateAnalysisReportAndRoadmap(
      assessment.user,
      extracted,
      orderedCourses
    );
