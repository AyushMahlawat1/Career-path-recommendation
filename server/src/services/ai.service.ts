...
export interface TraitScoresResponse {
  traits: {
    [traitName: string]: number; // Score from 1.0 to 10.0
  };
}
...
export interface CareerAnalysisResponse {
  ...
  weeklyStudyPlan: string[]; // Hourly weekly breakdown
...
function isGeminiResponse(obj: unknown): obj is GeminiResponse {
  ...
  if ('candidates' in res && res.candidates !== undefined && !Array.isArray(res.candidates)) return false;
  return true;
}
  ...
  "options": ["Option A", "Option B", "Option C"]
      ...
      if (response.ok) {
        ...
        if (isGeminiResponse(data)) {
          ...
          if (text) {
            return JSON.parse(text) as NextQuestionResponse;
          }
        }
      }
    } catch (error) {
    ...
    ...
      ...
      if (response.ok) {
        ...
        if (isGeminiResponse(data)) {
          const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
          if (text) {
            return JSON.parse(text) as TraitScoresResponse;
          }
        }
      }
    } catch (error) {
...
export async function generateAnalysisReportAndRoadmap(
  userProfile: { fullName: string; email: string },
  traitScores: { [traitName: string]: number },
  topCourses: Course[]
  ...
  "roadmap": [
    {
      ...
      if (response.ok) {
        ...
        if (isGeminiResponse(data)) {
          const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
          if (text) {
            return JSON.parse(text) as CareerAnalysisResponse;
          }
        }
      }
    } catch (error) {
...
function getMockNextQuestion(chatHistory: Array<{ role: string; text: string }>): NextQuestionResponse {
...
function extractMockTraits(chatHistory: Array<{ role: string; text: string }>): TraitScoresResponse {
